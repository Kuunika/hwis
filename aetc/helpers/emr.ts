...
export const findEncounterObs = (data:Encounter[], encounter:string, visit:number)=>{
...
export const formatAllAirwayBreathing = (obs:Obs[])=>{
  const airWayCompromised =  filterObservations(obs, concepts.IS_AIRWAY_COMPROMISED)
  const isBreathingNormal =  filterObservations(obs, concepts.IS_BREATHING_ABNORMAL)
  const oxygenStats =  filterObservations(obs, concepts.OXYGEN_STATS_89)
  const respiratoryRate =  filterObservations(obs, concepts.RESPIRATORY_RATE_8_31)
  const severeRespiratory =  filterObservations(obs, concepts.SEVERE_RESPIRATORY)
  const inabilityToSpeak =  filterObservations(obs, concepts.INABILITY_TO_SPEAK)
  const reducedConsciousness =  filterObservations(obs, concepts.REDUCED_LEVEL_CONSCIOUSNESS)
  ...
  const respiratoryRate92130 =  filterObservations(obs, concepts.RESPIRATORY_RATE_9_21_30)
...
export const formatAllCirculation = (obs: Obs[]) => {
  const isCirculationAbnormal = filterObservations(obs, concepts.IS_CIRCULATION_ABNORMAL);
  ...
  const reducedUrinaryOutput = filterObservations(obs, concepts.REDUCED_URINARY_OUTPUT);
...
export const formatAllConsciousness = (obs: Obs[]) => {
  const consciousness = filterObservations(obs, concepts.DOES_PATIENT_LOW_LEVEL_CONSCIOUSNESS);
...
export const formatAllPersistentPain = (obs: Obs[]) => {
  ...
  const focalNeurological = filterObservations(obs, concepts.FOCAL_NEUROLOGICAL);
