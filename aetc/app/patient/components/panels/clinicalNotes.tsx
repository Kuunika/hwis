...
export const ClinicalNotes = () => {
  ...
  useEffect(() => {
    ...
    // Fetch all observations (same as your existing code)
    ...
    // Add messages based on observations (same as your existing code)
    ...
    if (actionDoneObs?.value) {
      messages.push(`Action taken when the patient was actively bleeding: ${actionDoneObs.value}.`);
    }
    ...
    } else if (capillaryRefillTimeObs?.value == "3 seconds") {
    ...
    } else if (cprSiteObs?.value == "SSW") {
      messages.push("The site is SSW.");
    } else if (cprSiteObs?.value == "Priority") {
    ...
    if (cprRecord1DoseNameObs?.value) {
      messages.push(`Dose: ${cprRecord1DoseNameObs.value}.`);
    }
    ...
    if (cprSystolicObs?.value) {
      messages.push(`Systolic blood pressure: ${cprSystolicObs.value}.`);
    }
    if (cprDystoricObs?.value) {
      messages.push(`Diastolic blood pressure: ${cprDystoricObs.value}.`);
    }
    ...
    if (mucuouAbnormalObs?.value) {
      messages.push(`Mucous abnormality: ${mucuouAbnormalObs.value}.`);
    }
    ...
    if (bloodPressurSystolicObs?.value) {
      messages.push(`Systolic blood pressure: ${bloodPressurSystolicObs.value}.`);
    }
    if (bloodPressurDystolicObs?.value) {
      messages.push(`Diastolic blood pressure: ${bloodPressurDystolicObs.value}.`);
    }
    if (BPUnrecordableObs?.value) {
      messages.push(`BP is unrecordable: ${BPUnrecordableObs.value}.`);
    }
    if (bloodPressurNotDoneObs?.value) {
      messages.push(`BP was not done: ${bloodPressurNotDoneObs.value}.`);
    }
    ...
    // Join messages into a readable paragraph
    const formattedMessage = messages.join(" ");
  
    setCirculationMessage(formattedMessage);
  }, [pData]);
  ...
  useEffect(() => {
    ...
    // Join messages into a single paragraph
    const formattedMessage = messages.join(" ");
  
    setDisabilityMessage(formattedMessage);
  }, [pData]);
...
const [exposureMessage, setExposureMessage] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    setAdditionalFieldsMessage(formattedMessage);
  }, [pData]);
