...
export const PrimarySurvey = ({ panelData, renderTimestamp }: PrimarySurveyProps) => {
  // Define categories in a more maintainable way
  const airwayReasonFields = [
    concepts.SECRETION,
    concepts.TONGUE_SWELLING,
    concepts.NECK_HAEMATOMA,
    concepts.TONGUE_FALLING_BACK,
  ...
  const categorizeField = (fieldName: string) => {
    ...
    // Airway reasons (only shown if airway not patent)
    if (airwayReasonFields.includes(fieldName)) {
      return "airwayReasons";
    }

    // Airway interventions (always shown if present)
    if (airwayInterventionTerms.some(term => name.includes(term))) {
  ...
  const fields = extractAllFields(panelData);
  const isAirwayNotPatent = fields.core.some(
    f => f.name === concepts.AIRWAY_PATENT && f.value?.toLowerCase() === "no"
  );
  ...
  return (
    <Box sx={{ p: 2, minHeight: "60px" }}>
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} md={4}>
          {/* A – Airway */}
          {(fields.core.length > 0 || fields.airwayReasons.length > 0 || fields.airwayInterventions.length > 0) && (
            <>
              <Typography variant="body2" sx={{ fontWeight: "bold", mt: 1, mb: 0.5 }}>
                A – Airway with C-Spine Protection
              </Typography>
              ...
              {/* Airway Reasons (only shown if airway not patent) */}
              {isAirwayNotPatent && fields.airwayReasons.length > 0 && (
                <Box sx={{ pl: 2, mb: 1 }}>
                  <Typography variant="body2" sx={{ fontStyle: 'italic', mb: 0.5 }}>
                    Reasons:
                  </Typography>
        ...
        {/* Rest of your columns (C, D, E) remain exactly the same */}
        {/* Column 2: C & D */}
        <Grid item xs={12} md={4}>
          {/* C – Circulation */}
          {fields.circulation.length > 0 && (
