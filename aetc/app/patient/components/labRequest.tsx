...
export function LabRequestModal({ onClose, addRequest }: SimpleDialogProps) {
  ...
  const handleTestSelect = (tests: Array<number>) => {
    const mappedTests = tests.map((test) => {
      return {
        concept: labTests?.find((lab) => lab.concept_id == test)?.names[0]
          ?.uuid,
      };
    });
...
export const TestList = ({
  ...
  loading?: boolean;
  testTypes: TestType[];
  onSelectTest: (test: string | number) => void;
}) => {
      ...
      {loading ? (
        ...
        <>
          {testTypes.length > 0 ? (
            <ListSelect
              multiple={true}
              onSelectItem={onSelectTest}
              list={
                testTypes
                  ? testTypes.map((d) => ({ id: d.concept_id, label: d.name }))
                  : []
              }
              search={search}
            />
          ) : (
            <>No Tests on the selected sample</>
          )}
        </>
      )}
    </WrapperBox>
