...
const schema = Yup.object().shape({
  medications: Yup.array().of(
    Yup.object().shape({
      ...
      last_prescribed_date: Yup.string().required(
        "Date last prescribed is required"
      ),
      date_and_time_of_last_med: Yup.string().required(
        "Date and time last taken is required"
      ),
    })
  ),
});
...
export const MedicationsForm = ({ onSubmit, onSkip }: Prop) => {
  ...
  const frequencyLists = [
    ...
    {
      id: "8-Hourly-(TID)-Three-times-a-day",
      label: "8 Hourly (TID) -Three times a day",
    },
    {
      id: "6-Hourly-(QID)-Four-times-a-day",
      label: "6 Hourly (QID)- Four times a day",
    },
          ...
          <div key={index}>
            ...
            <FieldsContainer sx={{ mb: "1.5ch" }}>
              <FormControl sx={{ width: 300 }}>
                <InputLabel id={`formulation-label-${index}`}>
                  Formulation
                </InputLabel>
                <Select
                  ...
                  onChange={(e) =>
                    handleInputChange(index, "formulation", e.target.value)
                  }
                  label="Formulation"
                >
