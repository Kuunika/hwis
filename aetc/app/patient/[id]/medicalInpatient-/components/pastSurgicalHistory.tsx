...
const PastSurgicalHistoryPanel: React.FC<PastSurgicalHistoryPanelProps> = ({
  showForPrinting,
  toggleShow,
}: PastSurgicalHistoryPanelProps) => {
  const { params } = useParameters();
  const { data: historicData, isLoading: historyLoading } = getPatientsEncounters(
    params?.id as string
  );
  const [observations, setObservations] = useState<ProcessedObservation[]>([]);

  const displayedObservations = showForPrinting
    ? observations
    : observations.slice(0, 4);

  const sampleHistoryEncounters = historicData?.filter(
          ...
          const obsData: ProcessedObservation = {
            ...
            children: observation.children
              ? observation.children.map((child) => ({
                  ...
                  value_text: child.value_text ? child.value_text : child.value,
                  children: [],
                }))
              : [],
          };
  ...
  // Helper function to get surgery date from children
  const getSurgeryDate = (item: ProcessedObservation): string => {
    const dateChild = item.children.find(
      (child) => child.name === "Date of surgery"
    );
    
    if (dateChild?.value_text) {
      try {
        return new Date(dateChild.value_text).toLocaleDateString();
      } catch (error) {
        console.error("Invalid date:", dateChild.value_text);
        return "Invalid Date";
      }
    }
  ...
  // Helper function to get non-date children for display
  const getNonDateChildren = (item: ProcessedObservation) => {
    return item.children.filter((child) => child.name !== "Date of surgery");
  };
                ...
                <>
                  ...
                  >
                    {displayedObservations.map((item) => (
                      ...
                      >
                        {/* Display surgery type as main heading */}
                        <h3 style={{ marginBottom: "5px", color: "rgba(0, 0, 0, 0.8)" }}>
                          {item.value || item.name}
                        </h3>
                        
                        {/* Display surgery date */}
                        <h4 style={{ marginBottom: "10px" }}>
                          {getSurgeryDate(item)}
                        </h4>
                        
                        {/* Display other details */}
                        {getNonDateChildren(item).length > 0 && (
                          <ul style={{ paddingLeft: "15px" }}>
                            {getNonDateChildren(item).map((child) => (
                              <li key={child.obs_id}>
                                {(() => {
                                  let parsedValue = child.value_text;
                                  if (
                                    typeof child.value_text === "string" &&
                                    (child.value_text === "true" ||
                                      child.value_text === "false")
                                  ) {
