...
export const DisplaySocialHistory = ({
  socialHistory,
  loading,
  onSubmit,
  disabled,
}: {
  socialHistory: Encounter;
  loading: boolean;
  ...
  const maritalStatus: string = getObservationValue(
    socialHistory?.obs,
    concepts.MARITAL_STATUS
  );
  const religion: string = getObservationValue(
    socialHistory?.obs,
    concepts.RELIGION
  );
  const occupation: string = getObservationValue(
    socialHistory?.obs,
    concepts.OCCUPATION
  );
  const transportation: string = getObservationValue(
    socialHistory?.obs,
    concepts.METHOD_OF_TRANSPORTATION
  );
  const highestEducation: string = getObservationValue(
    socialHistory?.obs,
    concepts.HIGHEST_EDUCATION
  );
  ...
  return (
    <ContainerCard>
      <SocialHistoryDialog
        onSubmit={(values) => {
          onSubmit(values);
          setSocialHistoryDialogOpen(false);
        }}
        initialValues={{
        ...
        open={socialHistoryDialogOpen}
        onClose={() => setSocialHistoryDialogOpen(false)}
      />
      ...
      <MainButton
        ...
        sx={{ width: "10%" }}
        title="Edit"
        onClick={() => setSocialHistoryDialogOpen(true)}
      />
    </ContainerCard>
  );
};

export const DisplayFinancing = ({
  financing,
  loading,
  onSubmit,
  disabled,
  ...
  const [financingOption, setFinancingOptions] = useState({
    paymentOptions: "",
    insuranceProvider: "",
    insuranceNumber: "",
    insuranceScheme: "",
  ...
  useEffect(() => {
    const paymentOptions = getObservationValue(
      financing?.obs,
      "c7bcc8bd-09d5-4f98-8d58-5179f749fd99"
    );
    const insuranceProvider = getObservationValue(
      financing?.obs,
      "b0ffce26-2d25-449e-871e-c702e44bb37e"
    );
    const insuranceNumber = getObservationValue(
      financing?.obs,
      "98b08fb2-f877-45b6-a95a-db89dffefb27"
    );
    const insuranceScheme = getObservationValue(
      financing?.obs,
      "db2e6bba-7d04-4873-a0a2-ac7bd69dd7b1"
    );
    const insuranceStatus = getObservationValue(
      financing?.obs,
      "3cdd53d9-35a5-47e5-909f-654e5bc7c9a8"
    );
  ...
  return (
    <ContainerCard>
      <FinancingDialog
        ...
        onClose={() => setFinancingDialogOpen(false)}
      />
...
export const DisplayRelationship = ({
  ...
  const {
    data: relationships,
    isPending: loadingRelationships,
    refetch,
  ...
  const mappedRelationships: any = relationships?.map((relationship) => {
    ...
    const relationshipType = relationshipTypes?.find(
      (r) => r.relationship_type_id == relationshipId
    );

    const foundRelationshipType = relationshipTypes?.find(
      (type) => type.a_is_to_b == relationshipType?.b_is_to_a
    );
    ...
    return {
      ...
      relationshipUUID: foundRelationshipType?.uuid,
      person: relationship.person_b.uuid,
    };
  });
  ...
  const editRelationship = async (values: any) => {
    const mappedPatient = {
      identifiers: [],
      names: [
        {
  ...
  return (
    <WrapperBox
      sx={{
        display: "flex",
        width: mappedRelationships?.length == 1 ? "50%" : "100%",
      }}
    >
      <EditRelationship
        ...
        return (
          <ContainerCard>
            <MainTypography variant="h5">
              {relationship.relationship?.toLowerCase() == "guardian"
...
const ContainerCard = ({ children }: { children: ReactNode }) => {
  return (
    <MainPaper sx={{ p: "1ch", flex: 1, mx: "0.5ch" }}>{children}</MainPaper>
  );
};
