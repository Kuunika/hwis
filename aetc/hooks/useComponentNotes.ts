...
export const useComponentNotes = (encounterType: string) => {
  ...
  }, [filteredEncounters, visitDate, historyLoading, encounterType]);

  const formatComponentNotes = (
    obs: any[],
  ...
  const formatAirwayAssessmentNotes = (obs: any[]): ComponentNote[] => {
    ...
    obs.forEach((ob: any) => {
      ...
      if (name === "Airway Patent") {
        ...
        if (valueText === "Yes") {
          currentParagraph.push(
            "The patient was stabilised by applying the Neck Collar."
          );
        } else if (valueText === "No") {
          currentParagraph.push("The Neck Collar was not applied.");
        } else {
          currentParagraph.push(
            "Application of a Neck Collar was not indicated."
          );
        }
      } else if (name === "Head blocks applied") {
        if (valueText === "Yes") {
          currentParagraph.push(
            "Head blocks were applied to stabilize the C-Spine."
          );
        } else {
          currentParagraph.push(
            "Head blocks were not applied as an Intervention to stabilize the C-Spine."
          );
        }
      }
    });
  ...
  obs.forEach((ob: any) => {
    ...
    if (name === "Is Breathing Abnormal") {
      if (valueText === "Yes") {
        ...
        const startTimeObs = obs.find(
          (ob: any) => ob.names?.[0]?.name === "Start Time"
        );
        const endTimeObs = obs.find(
          (ob: any) => ob.names?.[0]?.name === "End Time"
        );

        if (startTimeObs) {
          addStatement(
            `The patient was assisted with ventilation from ${startTimeObs.value}`,
            true
          );
        }
        if (endTimeObs) {
...
const formatPresentingComplaintsNotes = (obs: any[]): ComponentNote[] => {
  ...
  sortedObs.forEach((item: any) => {
    if (
      item.names?.[0]?.name === "Current complaints or symptoms" &&
      item.value
    ) {
      ...
      if (
        !currentGroup ||
        new Date(itemTime).getTime() - new Date(currentGroup.time).getTime() >
          3 * 60 * 1000
      ) {
        currentGroup = {
          time: itemTime,
...
const formatMedicationsNotes = (obs: any[]): ComponentNote[] => {
  ...
  obs?.forEach((item: any) => {
    ...
    if (doseInInternationalUnits)
      parts.push(`Dose: ${doseInInternationalUnits} international units`);
    ...
    if (medicationFormulation)
      parts.push(`Medication formulation: ${medicationFormulation}`);
    if (lastTaken)
      parts.push(`Last taken: ${new Date(lastTaken).toLocaleDateString()}`);
    if (lastPrescription)
      parts.push(
        `Last prescribed: ${new Date(lastPrescription).toLocaleDateString()}`
      );
...
const formatExistingConditionsNotes = (obs: any[]): ComponentNote[] => {
  ...
  return groupedAssessments
    .map((group) => ({
      paragraph: group.conditions
        .map((condition) => createExistingConditionText(condition))
        .join(" "),
      time: group.time,
      creator: group.creator,
      rawTime: new Date(group.time).getTime(),
...
const formatSurgicalHistoryNotes = (obs: any[]): ComponentNote[] => {
  ...
  return groupedNotes
    .map((group) => ({
      paragraph: group.surgeries
        .map((surgery) => createSurgicalNoteText(surgery))
        .join(" "),
      time: group.time,
      creator: group.creator,
      rawTime: new Date(group.time).getTime(),
...
const createSurgicalNoteText = (surgery: {
  ...
  if (surgery.surgeryDate) {
    parts.push(
      `Date performed: ${new Date(surgery.surgeryDate).toLocaleDateString()}`
    );
  }
...
const formatAdmissionNotes = (obs: any[]): ComponentNote[] => {
    ...
    if (parentOb.names?.[0]?.name === "Admission date") {
      ...
      parentOb.children?.forEach((child: any) => {
        ...
        if (name === "Health center hospitals") {
          ...
          name === "Surgical interventions for tuberculosis" ||
  ...
  return groupedAdmissions
    .map((group) => ({
      paragraph: group.admissions
        .map((admission) => createAdmissionNoteText(admission))
        .join(" "),
      time: group.time,
      creator: group.creator,
      rawTime: new Date(group.time).getTime(),
...
const formatSystemsReviewData = (obs: any[]): ComponentNote[] => {
  ...
  obs.forEach((ob: any) => {
    ...
    if (
      name.startsWith("Review Of Systems") ||
      name.startsWith("Review of systems")
    ) {
      ...
      if (name === "Duration Of Symptoms Days") {
        if (currentSystem.symptoms.length > 0) {
          currentSystem.symptoms[currentSystem.symptoms.length - 1].duration =
            value;
        }
      } else if (name === "Anatomic locations") {
        if (currentSystem.symptoms.length > 0) {
          currentSystem.symptoms[currentSystem.symptoms.length - 1].location =
            value;
        }
      } else if (value === true || value === "true") {
        // This is a symptom
...
const formatHeadAndNeckNotes = (obs: any[]): ComponentNote[] => {
  ...
  const processObservation = (ob: any) => {
    ...
    if (name === "Image Part Name") {
      if (valueText === "Front") {
        if (
          currentAssessment.parts.length > 0 ||
          currentAssessment.abnormalities.size > 0
        ) {
          assessments.push(
            formatHeadAndNeckAssessmentToParagraph(currentAssessment)
          );
        }
      ...
      name === "Laceration length" &&
      currentAbnormalityType === "Laceration"
    ) {
      const lastAbnormality = getLastAbnormality(
        currentAssessment,
        currentImageName
      );
      if (lastAbnormality) lastAbnormality.details.length = valueText;
    } else if (
      name === "Laceration depth" &&
      currentAbnormalityType === "Laceration"
    ) {
      const lastAbnormality = getLastAbnormality(
        currentAssessment,
...
const formatHeadAndNeckAssessmentToParagraph = (
  assessment: any
): ComponentNote => {
  ...
  if (assessment.parts.length > 0) {
    paragraphParts.push(
      `Assessment of ${assessment.parts.join(", ")} was performed.`
    );
  }
  ...
  assessment.abnormalities.forEach((abnormalities: any, part: any) => {
    if (part === "General") {
      abnormalities.forEach((abnormality: any) => {
        paragraphParts.push(
          `Head and neck assessment status: ${abnormality.details.note}.`
        );
      });
    } else {
      abnormalities.forEach((abnormality: any) => {
        ...
        switch (abnormality.type) {
          case "Laceration":
            if (abnormality.details.length)
              description += ` (Length: ${abnormality.details.length}`;
            if (abnormality.details.depth)
              description += `, Depth: ${abnormality.details.depth}`;
            if (abnormality.details.other)
              description += `, Description: ${abnormality.details.other}`;
            if (abnormality.details.length) description += ")";
            break;
          case "Bruise":
            if (abnormality.details.description)
              description += ` (${abnormality.details.description})`;
            break;
          case "Other":
            if (abnormality.details.specify)
              description += ` (${abnormality.details.specify})`;
            break;
        }
    ...
    if (name === "Respiratory rate") {
      if (currentParagraph.length > 0) {
        if (abnormalZones.size > 0) {
          currentParagraph.push(
            `Abnormal findings in ${formatList(Array.from(abnormalZones))}.`
          );
      ...
      if (valueText === "Displaced") {
        const positioning = obs.find(
          (o) => o.names?.[0]?.name === "Positioning"
        )?.value;
        currentParagraph.push(
          "Apex beat displaced" + (positioning ? ` (${positioning}).` : ".")
        );
      } else {
      ...
      currentParagraph.push(
        valueText === "Yes" ? "Trill detected." : "No trill detected."
      );
    } else if (name === "Heaves") {
      currentParagraph.push(
        valueText === "Yes" ? "Heaves present." : "No heaves present."
      );
    } else if (name === "Chest Expansion") {
    ...
    .map((group) => {
      ...
      group.observations.forEach((ob) => {
        ...
        switch (name) {
          ...
          case "Bruit":
            assessment.findings[name] =
              valueText === "Yes" ? "Bruit present." : "Bruit not present.";
            break;
          ...
          case "Mass":
            assessment.findings[name] =
              valueText === "No" ? "Mass not present." : "Mass present.";
            break;
      ...
      if (assessment.additionalNotes.length > 0) {
        const cleanNotes = assessment.additionalNotes.filter(
    ...
    .map((group) => {
      ...
      group.observations.forEach((ob) => {
        ...
        if (name === "Abnormalities upper limb") {
          assessment.upperLimbStatus =
            valueText === "Yes" ? "abnormal" : "normal";
        } else if (name === "Abnormalities lower limb") {
          assessment.lowerLimbStatus =
            valueText === "Yes" ? "abnormal" : "normal";
        } else if (name === "Image Part Name") {
          ...
          if (isUpperLimb || isLowerLimb) {
              ...
              ob.children.forEach((child: any) => {
                if (child.children && child.children.length > 0) {
                  child.children.forEach((grandChild: any) => {
                    ...
                    if (
                      extremityAbnormalities.includes(abnormalityName) &&
                      grandChild.value === "Yes"
                    ) {
            ...
            if (abnormalities.length > 0) {
              if (isUpperLimb) {
                assessment.upperLimbImagesWithAbnormalities[imageName] =
                  abnormalities;
                assessment.upperLimbAbnormalities.push(...abnormalities);
              } else {
                assessment.lowerLimbImagesWithAbnormalities[imageName] =
                  abnormalities;
      ...
      if (assessment.upperLimbStatus === "normal") {
        ...
        Object.entries(assessment.upperLimbImagesWithAbnormalities).forEach(
          ([image, abnormalities]) => {
            parts.push(
              `Body part of ${image} shows ${abnormalities.join(", ")}`
            );
          }
        );
        ...
        if (assessment.upperLimbAbnormalities.length > 0) {
          const generalAbnormalities = assessment.upperLimbAbnormalities.filter(
            (ab) =>
              !Object.values(assessment.upperLimbImagesWithAbnormalities)
                .flat()
                .includes(ab)
          );
      ...
      if (assessment.lowerLimbStatus === "normal") {
        ...
        // Add abnormalities per image
        Object.entries(assessment.lowerLimbImagesWithAbnormalities).forEach(
          ([image, abnormalities]) => {
            parts.push(
              `Body part of ${image} shows ${abnormalities.join(", ")}`
            );
        ...
        if (assessment.lowerLimbAbnormalities.length > 0) {
          const generalAbnormalities = assessment.lowerLimbAbnormalities.filter(
            (ab) =>
              !Object.values(assessment.lowerLimbImagesWithAbnormalities)
                .flat()
                .includes(ab)
          );
          if (generalAbnormalities.length > 0) {
      ...
      if (assessment.additionalNotes.length > 0) {
        paragraphParts.push(
          `Additional notes: ${assessment.additionalNotes.join("; ")}`
        );
      }
...
const formatSoapierNotes = (obs: any[]): ComponentNote[] => {
  ...
  if (
    Object.keys(currentNote.sections).length > 0 ||
...
const createSoapierNote = (note: {
    ...
    if (note.sections[section]) {
      ...
      if (section === "Medical record observations") {
        ...
        if (Object.keys(note.vitals).length > 0) {
          ...
          if (
            note.vitals["Systolic blood pressure"] &&
            note.vitals["Diastolic blood pressure"]
          ) {
            vitalParts.push(
              `BP: ${note.vitals["Systolic blood pressure"]}/${note.vitals["Diastolic blood pressure"]} mmHg`
            );
          }
          ...
          if (note.vitals["Respiratory Rate"]) {
            vitalParts.push(
              `Respiratory Rate: ${note.vitals["Respiratory Rate"]} breaths/min`
            );
          }
          if (note.vitals["Spo2"]) {
        ...
        if (Object.keys(note.labs).length > 0) {
          ...
          if (note.labs["Malaria Rapid Diagnostic Test (MRDT)"]) {
            labParts.push(
              `Malaria Test: ${note.labs["Malaria Rapid Diagnostic Test (MRDT)"]}`
            );
          }
          ...
          if (note.labs["Urine Dipstick Ketones"]) {
            labParts.push(
              `Urine Ketones: ${note.labs["Urine Dipstick Ketones"]}`
            );
          }
        ...
        if (note.medications.length > 0) {
          const medParts = note.medications.map((med) => {
            ...
            if (med.dose && med.doseUnit)
              medStr += ` ${med.dose} ${med.doseUnit}`;
            if (med.frequency) medStr += ` ${med.frequency}`;
            if (med.duration && med.durationUnit)
              medStr += ` for ${med.duration} ${med.durationUnit}`;
  ...
  if (
    !note.sections["Medical record observations"] &&
    ...
    if (objectiveParts.length > 0) {
      const assessmentIndex = paragraphParts.findIndex((p) =>
        p.startsWith("Assessment:")
      );
      if (assessmentIndex >= 0) {
        paragraphParts.splice(
          assessmentIndex,
          0,
          "Objective:",
          ...objectiveParts
...
const createCirculationAssessmentNote = (note: {
  ...
  if (obs[concepts.PULSE_RATE]?.value == "Weak") {
    ...
    messages.push(
      `The patient's pulse rate is ${obs[concepts.PULSE_RATE].value}.`
    );
  }
  ...
  if (obs[concepts.CAPILLARY_REFILL_TIME]?.value == "Less than 3 seconds") {
    messages.push(
      "The patient's capillary refill time is less than 3 seconds."
    );
    ...
    messages.push(
      `The patient's capillary refill time is ${
        obs[concepts.CAPILLARY_REFILL_TIME].value
      }.`
    );
  }
  ...
  if (obs[concepts.SPECIFY]?.value) {
    messages.push(
      `Witnessed cardiac arrest specification: ${obs[concepts.SPECIFY].value}.`
    );
  }
  ...
  if (obs[concepts.REVERSIBLE_CAUSES]?.value) {
    messages.push(
      `Reversible causes: ${obs[concepts.REVERSIBLE_CAUSES].value}.`
    );
  }
  ...
  if (obs[concepts.SYSTOLIC_BLOOD_PRESSURE]?.value) {
    messages.push(
      `Systolic blood pressure: ${obs[concepts.SYSTOLIC_BLOOD_PRESSURE].value}.`
    );
  }
  ...
  if (obs[concepts.DIASTOLIC_BLOOD_PRESSURE]?.value) {
    messages.push(
      `Diastolic blood pressure: ${
        obs[concepts.DIASTOLIC_BLOOD_PRESSURE].value
      }.`
    );
  }
  ...
  if (obs[concepts.VERBAL_RESPONSE]?.value == "Oriented") {
    ...
    obs[concepts.VERBAL_RESPONSE]?.value == "Incomprehensible sounds"
  ) {
  ...
  if (obs[concepts.REASON_CPR_STOPPED]?.value) {
    messages.push(
      `Reason CPR stopped: ${obs[concepts.REASON_CPR_STOPPED].value}.`
    );
  }

  if (obs[concepts.DISPOSITION_AFTER_CPR]?.value) {
    messages.push(
      `Disposition after CPR: ${obs[concepts.DISPOSITION_AFTER_CPR].value}.`
    );
  }
  ...
  if (obs[concepts.MUCOUS_ABNORMAL]?.value) {
    messages.push(
      `Mucous abnormality: ${obs[concepts.MUCOUS_ABNORMAL].value}.`
    );
  }
  ...
  if (obs[concepts.BLOOD_PRESSURE_MEASURED]?.value == "Done") {
    ...
    if (
      obs[concepts.BLOOD_PRESSURE_SYSTOLIC]?.value &&
      obs[concepts.BLOOD_PRESSURE_DIASTOLIC]?.value
    ) {
      messages.push(
        `BP: ${obs[concepts.BLOOD_PRESSURE_SYSTOLIC].value}/${
          obs[concepts.BLOOD_PRESSURE_DIASTOLIC].value
        }.`
      );
    }
  ...
  if (obs[concepts.INTRAVENOUS]?.value == "Yes") {
    ...
    if (obs[concepts.SIZE_OF_CATHETER]?.value) {
      messages.push(
        `IV catheter size: ${obs[concepts.SIZE_OF_CATHETER].value}.`
      );
    }
    if (obs[concepts.CANNULATION_SITE]?.value) {
      messages.push(
        `Cannulation site: ${obs[concepts.CANNULATION_SITE].value}.`
      );
    }
  } else {
...
const createDisabilityAssessmentNote = (note: {
  ...
  if (obs[concepts.DOES_PATIENT_LOW_LEVEL_CONSCIOUSNESS]?.value === "No") {
    messages.push(
      "The patient is alert and does not exhibit a low level of consciousness."
    );
  } else {
    messages.push(
      "The patient exhibits a low level of consciousness and requires further evaluation and monitoring."
    );
  }
  // GCS
  if (obs[concepts.GCS]?.value === 15) {
    messages.push(
      "The GCS is 15: patient is fully conscious with normal neurological function."
    );
    ...
    messages.push(
      "GCS is 9–12: Moderate brain injury. Further assessment required."
    );
  } else if (obs[concepts.GCS]?.value >= 3 && obs[concepts.GCS]?.value <= 8) {
    messages.push(
      "GCS is 3–8: Severe brain injury or coma. Immediate intervention required."
    );
  } else {
  ...
  if (obs[concepts.EYE_OPENING_RESPONSE]?.value == 4) {
    ...
    messages.push(
      "Eyes open to pain: more significant impairment in consciousness."
    );
  ...
  if (obs[concepts.VERBAL_RESPONSE]?.value == 5) {
    messages.push(
      "Verbal response is 5: patient is oriented and converses normally."
    );
  } else if (obs[concepts.VERBAL_RESPONSE]?.value == 4) {
    messages.push(
      "Verbal response is 4: patient is confused but able to speak."
    );
  } else if (obs[concepts.VERBAL_RESPONSE]?.value == 3) {
    messages.push(
      "Verbal response is 3: inappropriate words, not making sense."
    );
  } else if (obs[concepts.VERBAL_RESPONSE]?.value == 2) {
    messages.push(
      "Verbal response is 2: incomprehensible sounds, moaning or groaning."
    );
  } else if (obs[concepts.VERBAL_RESPONSE]?.value == 1) {
    messages.push(
      "Verbal response is 1: no verbal response, patient is unresponsive."
    );
  } else {
  ...
  messages.push(
    `Pupil Size and Reaction to Light: ${
      obs[concepts.PUPIL_SIZE_AND_REACTION_TO_LIGHT]?.value || "Not available"
    }`
  );
  ...
  messages.push(
    `Focal Neurology: ${
      obs[concepts.FOCAL_NEUROLOGY]?.value || "Not available"
    }`
  );
...
const createExposureAssessmentNote = (note: {
  ...
  if (obs[concepts.ADDITIONAL_NOTES]?.value) {
    messages.push(
      `Additional Notes: ${obs[concepts.ADDITIONAL_NOTES].value}. `
    );
  }
  ...
  if (obs[concepts.ABNORMALITIES]?.value) {
    messages.push(
      `Skin abnormalities on Cephalic: ${obs[concepts.ABNORMALITIES].value}. `
    );
  }
